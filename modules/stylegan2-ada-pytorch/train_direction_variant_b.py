...
from generate_trainable_alphas import generate_image_from_w
...
def train_direction_for_target(
    ...
    """
    Train a semantic direction for a specific target attribute with learnable alphas.
    ...
    for step in range(steps):
        ...
        for w in w_list:
            # Generate image with current direction and alphas
    ...
    # Save trained directions and alphas
    torch.save({'directions': best_directions.cpu(), 'alphas': alphas.detach().cpu()}, 
               os.path.join(target_path, 'directions.pt'))
...
@click.option('--save-dir', default='../../outputs/training_steps_variant_b/', help='Directory to save results')
